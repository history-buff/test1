...
<%@include file="inc/common_head.jsp"%>
		<script type="text/javascript">
			...
			var code_flag=false;
        	 ...
        	 ajax({
 	     			url: "${path}/userServlet?method=checkEmail&email=" +email,
 	     			callback: function(result) {
	     				// 0: 不存在  其他情况存在
	     				if (result == "true") {
	     					username_notice.style.color = "green";
	     					username_notice.innerHTML = "可用！";
	     					email_flag= true;
	     				}
	     				// 重复
	     				else {
	     					username_notice.style.color = "red";
	     					username_notice.innerHTML = "用户名已存在，请重新输入！";
	     					email_flag = false;
	     				}
	     			}
	     		});          
	         ...
	         function checkCode(checkcode){
	        	 alert("aad");
	        	 ajax({
	 	     			url: "${path}/userServlet?method=checkCode&checkcode=" +checkcode,
	 	     			callback: function(result) {
		     				// 0: 不存在  其他情况存在
		     				if (result == "true") {
		     					captcha_notice.style.color = "green";
		     					captcha_notice.innerHTML = "正确！";
		     					code_flag= true;
		     				}
		     				// 重复
		     				else {
		     					captcha_notice.style.color = "red";
		     					captcha_notice.innerHTML = "验证码输入错误，请重新输入！";
		     					code_flag=false;
		     				}
		     			}
		     		});          
		...
		<div class="usBox">
			<div class="usBox_1">
				...
				<form action="${path}/userServlet?method=register" method="post" name="formUser"
					onsubmit="return register()&&mygo();"><!-- 校验输入无误后发送请求 --> 
					<table width="100%" border="0" align="left" cellpadding="5"
						...
						<tr>
							...
							<td><input type="text" size="8" name="checkcode" id="captcha"
								class="inputBg" onblur="if(check_captcha(this.value)){checkCode(this.value);};" /> <span style="color:#FF0000"
